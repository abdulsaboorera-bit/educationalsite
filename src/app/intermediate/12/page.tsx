...
export const metadata: Metadata = {
  title: "12th Class Intermediate - Subjects, Groups, Syllabus | PakEdu",
  description: "Complete guide to 12th class (Intermediate Part 2) in Pakistan. All groups, subjects, syllabus, past papers, and preparation resources.",
  keywords: ["12th class", "intermediate part 2", "second year", "12th class subjects", "12th class syllabus"],
  alternates: { canonical: `${SITE_CONFIG.url}/intermediate/12` },
  openGraph: {
    title: "12th Class Intermediate | PakEdu",
    description: "Complete guide to 12th class Intermediate in Pakistan.",
    url: `${SITE_CONFIG.url}/intermediate/12`,
    siteName: "PakEdu",
    locale: "en_PK",
    type: "website",
  },
...
export default function Class12Page() {
  const groups = getGroupsByClassLevel("12");
  const compulsory = getCompulsorySubjects("12");
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <section className="bg-gradient-to-br from-purple-600 via-purple-700 to-indigo-800 text-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
          ...
          />
          <h1 className="text-3xl md:text-4xl font-bold mt-6">12th Class (Intermediate Part 2)</h1>
          <p className="text-purple-100 mt-4 max-w-2xl text-lg">
            Your second and final year of Intermediate. Complete your HSSC preparation with advanced topics and board exam readiness.
          </p>
        </div>
      </section>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 space-y-12">
        ...
        <section className="bg-white rounded-2xl border border-slate-200 p-6 md:p-8">
          ...
          <p className="text-slate-500 text-sm mb-6">These subjects are studied by all 12th class students regardless of their group.</p>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {compulsory.map((subject) => (
              <Link
                key={subject.slug}
                href={`/intermediate/subjects/${subject.slug}`}
                className="bg-slate-50 rounded-xl border border-slate-200 p-4 hover:border-purple-300 hover:shadow-md transition-all"
              >
        ...
        {/* Groups */}
        <section>
          <h2 className="text-2xl font-bold text-slate-900 mb-2">Groups Available for Class 12</h2>
          <p className="text-slate-500 mb-6">Continue your chosen group with advanced-level topics in your second year.</p>
          <div className="space-y-6">
            {groups.map((group) => {
              const electives = getElectiveSubjects(group.slug, "12");
              return (
                    ...
                    <div className="flex items-start gap-4 mb-4">
                      ...
                      <div className="flex-1">
                        <Link href={`/intermediate/${group.slug}`} className="group/link">
                          <h3 className="text-xl font-bold text-slate-900 group-hover/link:text-purple-600 transition-colors">
                            {group.name}
                          </h3>
                        </Link>
